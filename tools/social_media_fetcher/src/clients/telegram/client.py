...
class TelegramFetcher(SocialMediaClient):
    ...
    async def fetch_raw_data(
        ...
        try:
            ...
            if not channel_info:
                raise RuntimeError(f"Telegram channel is unavailable: {source_id}")

            requested_limit = max(1, int(limit))
            raw_data_list: list[dict[str, Any]] = []
            offset_id = int(kwargs.get("offset_id", 0))
            min_id = max(0, int(kwargs.get("min_id", 0)))
                ...
                for message_group in messages:
                    try:
                        ...
                        if raw_data:
                            ...
                            raise RuntimeError("Telegram message conversion failed")
                    except Exception as e:
                        raise RuntimeError(
                            "Telegram message group conversion failed"
                        ) from e
            ...
            logger.exception("Error fetching raw data from {}", source_id)
            raise
    ...
    async def get_channel_messages_with_groups(
        ...
        *,
        ...
        try:
                ...
                if hasattr(original_msg, "grouped_id") and original_msg.grouped_id:
                    ...
                    if group_id not in grouped_messages:
                        ...
                        for other_orig, other_tg in all_messages:
                            if (
                                hasattr(other_orig, "grouped_id")
                                and other_orig.grouped_id == group_id
                                and other_tg.id not in processed_ids
                            ):
            ...
            raise RuntimeError(f"Telegram rate limit for {e.seconds} seconds") from e
        except (UsernameNotOccupiedError, ValueError) as error:
            raise RuntimeError(f"Telegram channel not found: {username}") from error
        except ChannelPrivateError as error:
            raise RuntimeError(f"Telegram channel is private: {username}") from error
    ...
    async def get_channel_info(self, username: str) -> TelegramChannelInfo | None:
        ...
        try:
            ...
            if message.media:
                if isinstance(message.media, MessageMediaPhoto):
                    ...
                    if media is None:
                        raise RuntimeError("Telegram photo conversion failed")
                    media_list.append(media)

                elif isinstance(message.media, MessageMediaDocument):
                    media = await self._convert_document_media(message)
                    if media is None:
                        raise RuntimeError("Telegram document conversion failed")
                    media_list.append(media)
            ...
            logger.exception("Telegram message conversion failed")
            raise

    async def _convert_photo_media(self, message: Message) -> TelegramMedia | None:
        ...
        try:
            ...
            if photo.sizes:
                largest_size = max(
                    photo.sizes, key=lambda s: getattr(s, "w", 0) * getattr(s, "h", 0)
                )
    ...
    async def _convert_document_media(self, message: Message) -> TelegramMedia | None:
    ...
    async def _convert_media(self, message: Message) -> TelegramMedia | None:
        ...
        try:
            ...
            if hasattr(message.media, "photo") and getattr(
            ...
            elif hasattr(message.media, "document") and getattr(
            ...
            elif hasattr(message.media, "webpage") and getattr(
                message.media, "webpage", None
            ):
                ...
                if hasattr(wp, "photo") and getattr(wp, "photo", None):
                    ...
                    try:
                        ...
                        file_unique_id = (
                            str(getattr(photo, "access_hash", None))
                            if getattr(photo, "access_hash", None) is not None
                            else None
                        )
            ...
            if not file_id:
                logger.debug(
                    f"Skipping media conversion - missing file_id. file_unique_id: {file_unique_id}"
                )
                return None
