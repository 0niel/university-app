...
class TelegramToNewsBlocksAdapter(SocialMediaToNewsBlocksAdapter):
    ...
    def _create_slideshow_blocks(
        ...
        *,
        ...
        cover_url = (
            photo_file_ids[0]
            if photo_file_ids
            else (video_file_ids[0] if video_file_ids else "")
        )
        ...
        for image_url in photo_file_ids:
            slides.append(
                SlideBlock(
                    ...
                    caption=self._generate_slide_caption(
                        media_index, "Фото", is_circles
                    ),
                    description="",
    ...
    def _generate_slide_caption(
        self, index: int, media_type: str, is_circles: bool
    ) -> str:
    ...
    def _create_media_blocks(
        ...
        for image_url in photo_file_ids[1:]:
            if image_url:
                blocks.append(
                    ImageBlock(type=ImageBlock.get_identifier(), image_url=image_url)
                )
        for video_url in video_file_ids:
            if video_url:
                blocks.append(
                    VideoBlock(type=VideoBlock.get_identifier(), video_url=video_url)
                )
        return blocks
