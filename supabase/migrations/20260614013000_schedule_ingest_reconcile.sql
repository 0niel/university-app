-- Add vanished-lesson reconciliation to the canonical ingest. On a target's
-- full_replace chunk, the payload carries the complete set of the target's part
-- UIDs (`source_uids`). We remove this target entity's edges to items no longer
-- in that set, then GC items left with zero edges (the change trigger logs
-- 'cancel'). The manifest is complete even though parts arrive chunked, so there
-- is no partial-state window.
...
declare
  ...
  v_full_replace boolean; v_manifest text[]; v_entity_id uuid; v_orphan_candidates uuid[];
  v_received int := 0; v_targets_up int := 0; v_items int := 0; v_skipped int := 0;
  v_occ int := 0;
begin
  perform p_source;
  perform p_sync_run_id;
  ...
  for v_target in select value from jsonb_array_elements(p_targets) loop
    ...
    -- ── reconcile vanished lessons for this target (full_replace chunk only) ──
    v_full_replace := coalesce((v_target ->> 'full_replace')::boolean, true);
    if v_full_replace and jsonb_typeof(v_target -> 'source_uids') = 'array' then
      v_manifest := array(select jsonb_array_elements_text(v_target -> 'source_uids'));
      if coalesce(array_length(v_manifest, 1), 0) > 0 then
        v_entity_id := case v_target_type
          when 'group' then (select id from core.schedule_groups where organization_id = p_organization_id and external_id = v_external_id)
          when 'teacher' then (select id from core.schedule_teachers where organization_id = p_organization_id and external_id = v_external_id)
          when 'classroom' then (select id from core.schedule_classrooms where organization_id = p_organization_id and external_id = v_external_id)
          else null end;
        if v_entity_id is not null then
          v_orphan_candidates := null;
          -- 1) remove this target entity's edges to items no longer in its feed,
          --    capturing the affected item ids (data-modifying CTE -> array).
          if v_target_type = 'group' then
            with del as (
              delete from core.schedule_item_group e using core.schedule_item si
              where e.group_id = v_entity_id and si.id = e.item_id
                and si.organization_id = p_organization_id and not (si.source_uid = any(v_manifest))
              returning e.item_id)
            select array_agg(item_id) into v_orphan_candidates from del;
          elsif v_target_type = 'teacher' then
            with del as (
              delete from core.schedule_item_teacher e using core.schedule_item si
              where e.teacher_id = v_entity_id and si.id = e.item_id
                and si.organization_id = p_organization_id and not (si.source_uid = any(v_manifest))
              returning e.item_id)
            select array_agg(item_id) into v_orphan_candidates from del;
          elsif v_target_type = 'classroom' then
            with del as (
              delete from core.schedule_item_classroom e using core.schedule_item si
              where e.classroom_id = v_entity_id and si.id = e.item_id
                and si.organization_id = p_organization_id and not (si.source_uid = any(v_manifest))
              returning e.item_id)
            select array_agg(item_id) into v_orphan_candidates from del;
          end if;
          -- 2) GC items that just lost their last edge (separate statement so the
          --    not-exists checks see the edge deletions above).
          if v_orphan_candidates is not null then
            delete from core.schedule_item s2 where s2.id = any(v_orphan_candidates)
              and not exists (select 1 from core.schedule_item_group g where g.item_id = s2.id)
              and not exists (select 1 from core.schedule_item_teacher t where t.item_id = s2.id)
              and not exists (select 1 from core.schedule_item_classroom c where c.item_id = s2.id);
          end if;
        end if;
      end if;
    end if;
  ...
  return jsonb_build_object('organization_id', p_organization_id, 'targets_received', v_received,
    'targets_upserted', v_targets_up, 'items_upserted', v_items, 'items_skipped', v_skipped,
    'occurrences_upserted', v_occ);
end; $$;
