create or replace function ingest_v1.upsert_schedule_payload(p_organization_id text, p_source jsonb, p_targets jsonb, p_sync_run_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SET search_path TO ''
AS $function$
    ...
    for v_part in select value from jsonb_array_elements(v_target -> 'parts') loop
      ...
      on conflict (organization_id, term_id, source_uid) do update
        ...
        where core.schedule_item.content_hash is distinct from excluded.content_hash
           or core.schedule_item.lesson_type is distinct from excluded.lesson_type
