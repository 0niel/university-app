...
class RtuMireaScheduleApiClient {
  ...
  RtuMireaScheduleApiClient({http.Client? httpClient})
    : this._(baseUrl: 'https://schedule-of.mirea.ru', httpClient: httpClient);
  ...
  Future<SearchData> search({required String query, int limit = 15}) async {
    final uri = Uri.parse('$_baseUrl/schedule/api/search').replace(
      queryParameters: <String, String>{'limit': '$limit', 'match': query},
    );
  ...
  Map<String, String> _getRequestHeaders() => const {
...
class RtuMireaScheduleApiMalformedResponse implements Exception {
  ...
  @override
  String toString() => 'RtuMireaScheduleApiMalformedResponse: $error';
}
...
class RtuMireaScheduleApiRequestFailure implements Exception {
  ...
  @override
  String toString() =>
      'RtuMireaScheduleApiRequestFailure: HTTP $statusCode ($body)';
}
