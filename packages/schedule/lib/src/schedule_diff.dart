...
ScheduleUpdateDiff computeScheduleDiff(
  ...
  String keyFor(LessonSchedulePart l) {
    ...
    final startTime = l.lessonBells.startTime;
    final endTime = l.lessonBells.endTime;
  ...
  for (final key in commonKeys) {
    final olds = (oldByKey[key] ?? <LessonSchedulePart>[])
      ..sort((a, b) => _dateListHash(a).compareTo(_dateListHash(b)));
    final news = (newByKey[key] ?? <LessonSchedulePart>[])
      ..sort((a, b) => _dateListHash(a).compareTo(_dateListHash(b)));

    final maxLen = olds.length > news.length ? olds.length : news.length;
    for (var i = 0; i < maxLen; i++) {
      final oldL = olds.elementAtOrNull(i);
      final newL = news.elementAtOrNull(i);
  ...
  for (final key in onlyNew) {
    for (final l in newByKey[key] ?? const <LessonSchedulePart>[]) {
      added.add(
        LessonChangeDetail(
          kind: .added,
          subject: l.subject,
  ...
  for (final key in onlyOld) {
    for (final l in oldByKey[key] ?? const <LessonSchedulePart>[]) {
      removed.add(
        LessonChangeDetail(
          kind: .removed,
          subject: l.subject,
...
void _mergeAddRemovePairsIntoModified(
  ...
  String? datesValue(LessonChangeDetail d) {
    final dates = d.fieldChanges.firstWhereOrNull(
      (c) => c.field == .dates,
    );
  ...
  for (final r in removed.reversed.toList()) {
    final rDates = datesValue(r);
    final j = added.indexWhere(
      (a) => a.subject == r.subject && datesValue(a) == rDates,
    );
    if (j == -1) continue;

    final a = added.elementAtOrNull(j);
    if (a == null) continue;
    ...
    for (final field in [
      ...
      if (ov != nv) {
        fieldDiffs.add(
          LessonFieldChange(field: field, oldValue: ov, newValue: nv),
        );
      }
    }
