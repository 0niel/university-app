...
void main() {
  group('BannerAdContent', () {
    ...
    testWidgets(
      ...
      await tester.pumpApp(
        BannerAdContent(
          ...
          adBuilder: adBuilder,
          currentPlatform: platform,
          anchoredAdaptiveAdSizeProvider: (orientation, width) async => AnchoredAdaptiveBannerAdSize(
    ...
    testWidgets('uses AdSize.mediumRectangle for BannerAdSize.large', (tester) async {
    ...
    testWidgets('uses AdSize(300, 600) for BannerAdSize.extraLarge', (tester) async {
      const expectedSize = AdSize(width: 300, height: 600);
      when(() => ad.size).thenReturn(expectedSize);

      await tester.pumpApp(
        BannerAdContent(
          size: BannerSize.extraLarge,
          adBuilder: adBuilder,
          currentPlatform: platform,
        ),
      );
    ...
    testWidgets(
      ...
      expect(
        ...
        equals([
          ...
          for (var i = 1; i <= adsRetryPolicy.maxRetryCount; i++) isA<BannerAdFailedToLoadException>(),
        ]),
      );
    });
