...
GoRouter createRouter() => GoRouter(
  ...
  redirect: (context, state) {
    // Check if we need to show splash video
    final splashVideoBloc = context.read<SplashVideoBloc>();
    final splashVideoState = splashVideoBloc.state;

    if (splashVideoState is SplashVideoReady && state.matchedLocation != '/splash-video') {
      return '/splash-video';
    }
  ...
  routes: [
    ...
    GoRoute(
      ...
      builder: (context, state) {
        ...
        if (splashVideoState is SplashVideoReady) {
          return SplashVideoPage(videoPath: splashVideoState.videoPath);
        }

        // If video is not ready or already shown, redirect to home
    ...
    StatefulShellRoute.indexedStack(
      ...
      branches: [
        ...
        StatefulShellBranch(
          ...
          routes: [
            GoRoute(
              ...
              routes: [
                GoRoute(path: '/custom', builder: (context, state) => const CustomSchedulesPage()),
                GoRoute(path: 'search', builder: (context, state) => SearchPage(query: state.extra as String?)),
                GoRoute(
                  path: 'details',
                  redirect: (context, state) {
                    try {
          ...
          routes: [
            GoRoute(
              ...
              routes: [
                ...
                GoRoute(path: 'map', builder: (context, state) => const MapPageView()),
                GoRoute(
                ...
                GoRoute(
                  ...
                  routes: [
                    GoRoute(path: 'about', builder: (context, state) => const AboutRatingSystemPage()),
                    GoRoute(path: 'subject', builder: (context, state) => SubjectPage(subject: state.extra as Subject)),
                  ],
                ),
        ...
        StatefulShellBranch(
          ...
          routes: [
            GoRoute(
              ...
              routes: [
                GoRoute(path: 'schedule-management', builder: (context, state) => const ScheduleManagementPage()),
                GoRoute(path: 'about', builder: (context, state) => const AboutAppPage()),
                GoRoute(path: 'announces', builder: (context, state) => const ProfileAnnouncesPage()),
                GoRoute(
                  path: 'settings',
        ...
        StatefulShellBranch(
          ...
          routes: [GoRoute(path: '/info', builder: (context, state) => const AboutAppPage())],
        ),
      ],
    ),
    GoRoute(
    ...
    GoRoute(
      ...
      pageBuilder:
          (context, state) => CustomTransitionPage(
            ...
            opaque: false,
            transitionsBuilder: (_, __, ___, child) => child,
            child: ImagesViewGallery(imageUrls: List<String>.from(state.extra as List<dynamic>)),
          ),
    ),
  ],
  observers: [
    FirebaseAnalyticsObserver(analytics: FirebaseAnalytics.instance),
    SentryNavigatorObserver(autoFinishAfter: const Duration(seconds: 5), setRouteNameAsTransaction: true),
  ],
);
