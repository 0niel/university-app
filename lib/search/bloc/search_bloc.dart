...
EventTransformer<Event> restartableDebounce<Event>(Duration duration, {required bool Function(Event) isDebounced}) {
...
class SearchBloc extends HydratedBloc<SearchEvent, SearchState> {
  SearchBloc({required ScheduleRepository scheduleRepository})
    ...
    }, transformer: restartableDebounce(_duration, isDebounced: (event) => event.searchQuery.isNotEmpty));
    on<AddQueryToSearchHistory>((event, emit) {
      if (state.searchHisoty.contains(event.query)) return;

      emit(state.copyWith(searchHisoty: [event.query, ...state.searchHisoty.take(4)]));
    });
    on<ClearSearchHistory>((event, emit) {
      emit(state.copyWith(searchHisoty: const []));
    });
    on<RemoveQueryFromSearchHistory>((event, emit) {
      emit(state.copyWith(searchHisoty: state.searchHisoty.where((e) => e != event.query).toList()));
    });
    on<ChangeSearchMode>((event, emit) {
  ...
  FutureOr<void> _onSearchQueryChanged(SearchQueryChanged event, Emitter<SearchState> emit) async {
    ...
    try {
      ...
      emit(
        state.copyWith(
          groups:
              state.searchMode == SearchMode.all || state.searchMode == SearchMode.groups
                  ? results[state.searchMode == SearchMode.all ? 0 : 0] as SearchGroupsResponse
                  : state.groups,
          teachers:
              state.searchMode == SearchMode.all || state.searchMode == SearchMode.teachers
                  ? results[state.searchMode == SearchMode.all ? 1 : 0] as SearchTeachersResponse
                  : state.teachers,
          classrooms:
              state.searchMode == SearchMode.all || state.searchMode == SearchMode.classrooms
