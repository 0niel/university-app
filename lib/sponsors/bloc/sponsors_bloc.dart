...
class SponsorsBloc extends Bloc<SponsorsEvent, SponsorsState> {
  SponsorsBloc({required CommunityRepository communityRepository})
  ...
  FutureOr<void> _onSponsorsLoadRequest(SponsorsLoadRequest event, Emitter<SponsorsState> emit) async {
    emit(state.copyWith(status: SponsorsStatus.loading));
    try {
      final results = await _communityRepository.getSponsors();

      emit(state.copyWith(sponsors: results, status: SponsorsStatus.loaded));
