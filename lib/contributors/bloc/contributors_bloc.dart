...
class ContributorsBloc extends Bloc<ContributorsEvent, ContributorsState> {
  ContributorsBloc({required CommunityRepository communityRepository})
  ...
  FutureOr<void> _onContributorsLoadRequest(ContributorsLoadRequest event, Emitter<ContributorsState> emit) async {
    emit(state.copyWith(status: ContributorsStatus.loading));
    try {
      final results = await _communityRepository.getContributors();

      emit(state.copyWith(contributors: results, status: ContributorsStatus.loaded));
