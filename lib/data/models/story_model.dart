...
class StoryPageModel extends StoryPage {
  const StoryPageModel({required title, required text, required media, required actions})
...
class StoryPageActionModel extends StoryPageAction {
  const StoryPageActionModel({required title, required url}) : super(title: title, url: url);

  factory StoryPageActionModel.fromJson(Map<String, dynamic> json) =>
      StoryPageActionModel(title: json["title"], url: json["url"]);

  Map<String, dynamic> toJson() => {"title": title, "url": url};
}

class AuthorModel extends Author {
  const AuthorModel({required name, required url, required logo}) : super(name: name, url: url, logo: logo);
