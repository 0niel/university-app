...
class FormatsModel extends Formats {
  const FormatsModel({required large, required small, required medium, required thumbnail})
    : super(large: large, small: small, medium: medium, thumbnail: thumbnail);
...
class StrapiImageModel extends StrapiImage {
  const StrapiImageModel({required url, required name, required size, required width, required height})
    : super(url: url, name: name, size: size, width: width, height: height);
  ...
  Map<String, dynamic> toJson() => {"url": url, "name": name, "size": size, "width": width, "height": height};
}
