...
описано ниже, работает внутри `appStateScope`. Экран без явной области
автоматически получает её. При изменении стейта обновляются узлы с изменившимися
значениями; остальные виджеты сохраняют свои экземпляры.
:::

Выражения в действиях вычисляются перед выполнением каждого шага. Поэтому
`onResult` после выбора файла или запроса уже видит новое значение
<span v-pre>`{{state.photo}}`</span>. Вложенные области состояния независимы.
При обновлении страницы новые серверные `initial` применяются к значениям,
которые пользователь ещё не менял; локальный черновик сохраняется.
