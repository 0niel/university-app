...
class ShorebirdPatchWorkflowTest(unittest.TestCase):
    ...
    def test_only_new_android_base_reconstructs_google_services_from_pinned_generator(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            for platform, version in (('android', '5.2.1+1006601'), ('ios', '5.2.1+2439.17.57'), ('android', '5.2.1+1006201'), ('ios', '5.2.1+2439.15.54'), ('android', '5.2.1+1005801'), ('android', '5.2.1+1005301'), ('android', '5.2.0+1004201'), ('ios', '5.2.0+2435.13.10'), ('ios', '5.2.1+2439.14.43')):
                with self.subTest(platform=platform, version=version):
                    ...
                    self.assertEqual(result.returncode, 0, result.stderr)
                    new_base = version in ('5.2.1+1005801', '5.2.1+1006201', '5.2.1+1006601')
    ...
    def test_latest_mobile_releases_are_available_with_exact_baselines(self):
        targets = (
            ("android", "5.2.1+1006601", "9369a9bc8f3bac521c1efb10942f76a4696ca279"),
            ("ios", "5.2.1+2439.17.57", "9369a9bc8f3bac521c1efb10942f76a4696ca279"),
        )
        for platform, version, baseline in targets:
            with self.subTest(version=version):
                for workflow, job, name in ((PATCH, "validate", "Validate requested target"), (PROMOTE, "promote", "Validate requested promotion")):
    ...
    def test_mobile_promotion_checks_pinned_runtime_and_baseline(self):
        script = step(PROMOTE, 'promote', 'Verify receipt')['run'].split("python3 - <<'PY'\n", 1)[1].rsplit('\nPY', 1)[0]
        assignments = {node.targets[0].id: ast.literal_eval(node.value) for node in ast.parse(script).body
            if isinstance(node, ast.Assign) and isinstance(node.targets[0], ast.Name)
            and node.targets[0].id in ('mobile_reviewed', 'mobile_baselines')}
        tool_tree = ast.parse((ROOT / 'tool/prepare_shorebird_patch.py').read_text())
        tool_pins = {node.targets[0].id: ast.literal_eval(node.value) for node in tool_tree.body
            if isinstance(node, ast.Assign) and isinstance(node.targets[0], ast.Name)
            and node.targets[0].id in ('MOBILE_BASELINES', 'MOBILE_REVIEWED_SHA')}
        self.assertEqual(assignments['mobile_reviewed'], tool_pins['MOBILE_REVIEWED_SHA'])
        self.assertEqual(assignments['mobile_baselines'], tool_pins['MOBILE_BASELINES'])
        with tempfile.TemporaryDirectory() as temporary:
            directory = Path(temporary) / 'promotion-receipt'
            directory.mkdir()
            for version, baseline in tool_pins['MOBILE_BASELINES'].items():
                receipt = dict(source_sha='a' * 40, app_id='app', platform='android', release_version=version,
                    patch_number=1, staging_run_id='123', track='staging', patch_id=1, artifacts=[{'hash': 'artifact'}],
                    baseline_sha=baseline, reviewed_source_sha=tool_pins['MOBILE_REVIEWED_SHA'], projection_sha256='b' * 64)
                for mutation in ({}, {'baseline_sha': 'c' * 40}, {'reviewed_source_sha': 'c' * 40}, {'projection_sha256': 'invalid'}):
                    with self.subTest(version=version, mutation=mutation):
                        (directory / 'shorebird-patch-receipt.json').write_text(json.dumps({**receipt, **mutation}))
                        result = subprocess.run([sys.executable, '-c', script], env={**os.environ,
    ...
    def test_deleted_runtime_is_excluded_from_analysis_and_test_arguments(self):
        self.assertIn("--diff-filter=ACMR", step(PATCH, "validate", "Analyze hotfix")["run"])
        self.assertIn("--diff-filter=ACMR", step(PATCH, "validate", "Test reviewed runtime changes")["run"])
